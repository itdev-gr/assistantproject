...
const KIND_LABEL: Record<ReconcileIssue['kind'], { en: string; el: string }> = {
  ...
  legacy_partnership_subscription: { en: 'Legacy partnership subscription', el: 'Παλιά συνδρομή συνεργασίας' },
...
export default async function AdminBillingPage({ params }: Props) {
  ...
  const [{ data: runs }, pendingEvents, paying, exempt] = await Promise.all([
    admin
      ...
      .limit(30),
    admin.from('stripe_webhook_events').select('id', { count: 'exact', head: true }).is('processed_at', null),
    admin.from('businesses').select('id', { count: 'exact', head: true }).in('billing_status', ['active', 'past_due']).eq('billing_exempt', false),
    admin.from('businesses').select('id', { count: 'exact', head: true }).eq('billing_exempt', true),
  ]);
  const latest = runs?.[0] ?? null;
  const issues = ((latest?.issues ?? []) as unknown as ReconcileIssue[]) ?? [];
  const summary = (latest?.summary ?? null) as unknown as (ReconcileSummary & { foundBeforeHealing?: number }) | null;
  const fmt = (iso: string) =>
    new Date(iso).toLocaleString(locale === 'en' ? 'en-GB' : 'el-GR', { dateStyle: 'medium', timeStyle: 'short' });
    ...
    <div>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          value={latest ? (latest.ok ? t('Clean', 'Καθαρό') : `${latest.issue_count} ${t('issues', 'θέματα')}`) : '—'}
        />
        <StatCard label={t('Paying businesses', 'Επιχειρήσεις με συνδρομή')} value={paying.count ?? 0} />
        <StatCard label={t('Exempt businesses', 'Απαλλαγμένες')} value={exempt.count ?? 0} />
        <StatCard label={t('Webhook events pending', 'Events σε εκκρεμότητα')} value={pendingEvents.count ?? 0} />
      </div>

      {latest && (
        <p className="mb-4 text-sm text-muted-foreground">
          {t('Ran', 'Έτρεξε')} {fmt(latest.ran_at)} ·{' '}
          {t('found', 'βρέθηκαν')} {summary?.foundBeforeHealing ?? latest.issue_count} ·{' '}
          {t('healed', 'διορθώθηκαν')} {latest.healed_count} ·{' '}
          {t('remaining', 'απομένουν')} {latest.issue_count}
          {summary && (
            <>
              {' '}· {t('Stripe live subscriptions', 'Ενεργές συνδρομές Stripe')} {summary.liveSubscriptionsInStripe} ·{' '}
            ...
            <EmptyState
              message={
                latest
                  ...
                  : t('No audit has run yet. Click “Run audit now”.', 'Δεν έχει τρέξει έλεγχος ακόμη. Πατήστε «Εκτέλεση ελέγχου».')
              }
            />
          ) : (
            <>
              ...
              {issues.map((issue, i) => (
                <div key={`${issue.kind}-${issue.entity.id}-${i}`} className={`grid grid-cols-[9rem_1fr_auto] items-start gap-3 px-4 py-3 ${tableRow}`}>
                  <div className="space-y-1">
                    <Pill tone={tone[issue.severity]}>{issue.severity}</Pill>
                    <p className="text-xs font-medium">{KIND_LABEL[issue.kind]?.[locale === 'en' ? 'en' : 'el'] ?? issue.kind}</p>
                  </div>
                  <div className="min-w-0 text-[13px]">
                    ...
                    {(issue.expected !== undefined || issue.actual !== undefined) && (
                      <p className="mt-0.5 font-mono text-[11px] text-muted-foreground">
                        {t('expected', 'αναμενόμενο')}: {String(issue.expected ?? '—')} · {t('actual', 'πραγματικό')}: {String(issue.actual ?? '—')}
                      </p>
                    )}
                  </div>
                  <div>{issue.fix ? <FixButton fix={issue.fix} locale={locale} /> : <span className="text-xs text-muted-foreground">{t('manual', 'χειροκίνητα')}</span>}</div>
                </div>
              ))}
            </>
          )}
        ...
        <TableFrame minWidth="min-w-[520px]">
          {(runs ?? []).length === 0 ? (
            ...
            <>
              ...
              {(runs ?? []).map((r) => (
                <div key={r.id} className={`grid grid-cols-[12rem_6rem_6rem_6rem] items-center gap-3 px-4 py-2 text-sm ${tableRow}`}>
                  <span>{fmt(r.ran_at)}</span>
                  <Pill tone={r.ok ? 'ok' : 'danger'}>{r.ok ? t('Clean', 'Καθαρό') : t('Issues', 'Θέματα')}</Pill>
